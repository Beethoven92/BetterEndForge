...
public class GeyserFeature extends Feature<NoFeatureConfig>
{
			...
			NoFeatureConfig config) 
	{
		...
		if (pos.getY() > 57) 
		{
			...
			obj1.setBlock(Blocks.WATER.getDefaultState());
			obj2.setBlock(Blocks.WATER.getDefaultState());
			...
			BlockHelper.setWithoutUpdate(world, pos, Blocks.WATER.getDefaultState());
			Mutable mut = new Mutable().setPos(pos);
			count = FeatureHelper.getYOnSurface(world, pos.getX(), pos.getZ()) - pos.getY();
			for (int i = 0; i < count; i++)
			{
				BlockHelper.setWithoutUpdate(world, mut, Blocks.WATER.getDefaultState());
				for (Direction dir: BlockHelper.HORIZONTAL_DIRECTIONS) {
					BlockHelper.setWithoutUpdate(world, mut.offset(dir), Blocks.WATER.getDefaultState());
				}
				mut.setY(mut.getY() + 1);
			}
