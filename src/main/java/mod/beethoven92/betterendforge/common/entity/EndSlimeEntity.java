...
public class EndSlimeEntity extends SlimeEntity {
	private static final DataParameter<Boolean> MOSSY = EntityDataManager.createKey(EndSlimeEntity.class, DataSerializers.BOOLEAN);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<PlayerEntity>(this, PlayerEntity.class, 10, true, false, (livingEntity) -> {
	...
	public ILivingEntityData onInitialSpawn(IServerWorld world, DifficultyInstance difficulty, SpawnReason spawnReason, ILivingEntityData entityData, CompoundNBT entityTag) {
		...
		if (ModBiomes.getFromBiome(world.getBiome(getPosition())) == ModBiomes.FOGGY_MUSHROOMLAND) {
			this.setMossy(true);
		}
	...
	protected void registerData() {
		super.registerData();
		this.dataManager.register(MOSSY, false);
	}
	...
	public void remove() {
		...
		if (!this.world.isRemote && i > 1 && this.dead) {
			...
			for (int l = 0; l < k; ++l) {
				...
				slimeEntity.setLocationAndAngles(this.getPosX() + (double) g, this.getPosY() + 0.5D, this.getPosZ() + (double) h, this.rand.nextFloat() * 360.0F, 0.0F);
	...
	protected void dropLoot(DamageSource source, boolean causedByPlayer) {
		...
		if (causedByPlayer && this.attackingPlayer != null) {
			int looting = EnchantmentHelper.getLootingModifier(this.attackingPlayer);
			minCount += looting;
		}
	...
	public boolean isMossy() {
		return this.dataManager.get(MOSSY);
	}
	...
	private static boolean notManyEntities(IServerWorld world, BlockPos pos, int radius, int maxCount) {
		...
		List<EndSlimeEntity> list = world.getEntitiesWithinAABB(EndSlimeEntity.class, box, (entity) -> { return true; });
		...
		public boolean shouldExecute() {
			return EndSlimeEntity.this.getAttackTarget() == null
					&& (EndSlimeEntity.this.onGround
							...
							|| EndSlimeEntity.this.isInLava()
							|| EndSlimeEntity.this.isPotionActive(Effects.LEVITATION))
					&& EndSlimeEntity.this.getMoveHelper() instanceof EndSlimeMoveControl;
		}
	...
	class FaceTowardTargetGoal extends Goal {
		...
		public boolean shouldExecute() {
			...
			if (livingEntity == null) {
				...
				return livingEntity instanceof PlayerEntity && ((PlayerEntity) livingEntity).abilities.disableDamage ? false : EndSlimeEntity.this.getMoveHelper() instanceof EndSlimeMoveControl;
			}
		}
		...
		public void tick() {
			...
			((EndSlimeMoveControl) EndSlimeEntity.this.getMoveHelper()).look(EndSlimeEntity.this.rotationYaw, EndSlimeEntity.this.canDamagePlayer());
		}
	}
